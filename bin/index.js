...
//GET THE OUTPUT PATH FOR THE IMAGES
...
function isMoreMatch(){
  let i;
  for(i=0; i<pk.length; i++){
    let file = weightedTraits[i][pk[i]];
    if(weights[file] <= 0)break;
  }
  if(i==pk.length) return true;
  return false;
}

function reduceAppearCount() {
  for(let i=0; i<pk.length; i++){
    let file = weightedTraits[i][pk[i]];
    if(weights[file] > 0) weights[file]--;
  }
}

function addGeneratedTraits() {
  let temp = [];
  for(let j=0; j<pk.length; j++)
    temp.push(pk[j]);
  generatedTraits.push(temp);
}

function pickId(traitId) {
  for(let i=0; i<traitFileNum[traitId]; i++){
      pk[traitId] = i;
      if( traitId<traits-1 ) pickId(traitId+1);
      else if(traitId==pickCnt-1) {
          if(isMoreMatch()){
            reduceAppearCount();
            addGeneratedTraits();
          }          
...
async function generateWeightedTraits() {
  for (const trait of traits) {
    ...
    traitFileNum.push(files.length);
